...
import type { MusicDistributionBenefitsContent } from "@/data/services/music-distribution";

import { FadeUp } from "@/components/services/fade-up";
import { cn } from "@/lib/utils";

/** Corner graphic, scaled to the slightly smaller card. */
const BENEFIT_CARD_IMAGE_W = 248;
const BENEFIT_CARD_IMAGE_H = 226;

/**
 * Full-width copy, bottom-right art. Gradient reads darker / recessed toward the top and
 * opens to a lighter bottom. No inner scroll so page wheel/touch scroll isn’t trapped.
 */
function BenefitBentoCard({
  title,
  description,
  imageSrc,
  imageAlt,
...
export function MusicDistributionBenefitsSection({
  content,
  className,
  hideTopRule = false
}: {
  content: MusicDistributionBenefitsContent;
  className?: string;
  hideTopRule?: boolean;
}) {
        ...
        <FadeUp>
          <header className="mx-auto mb-8 max-w-full px-2 text-center sm:mb-10 md:mb-11 sm:px-4">
            <h2
              ...
              id="music-distribution-benefits-heading"
            >
        ...
        <ul className="m-0 grid list-none grid-cols-1 gap-8 p-0 md:grid-cols-2 md:gap-8 lg:gap-10">
          {content.items.map((item, index) => {
            ...
            return (
              ...
              >
                <FadeUp
                  className={cn(centerLast && "w-full md:max-w-[calc(50%-1rem)] lg:max-w-[calc(50%-1.25rem)]")}
                  delay={index * 0.06}
                >
                  <BenefitBentoCard
